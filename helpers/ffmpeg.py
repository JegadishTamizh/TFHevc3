...
async def extract_audio(client, message, data):
    ...
    if data['name'] == "mkv":
        out, err, rcode, pid = await execute(f"ffmpeg -i '{dwld_loc}' -vf crop=1280:536 -c:v libx265 -crf 23 -preset slow -c:a copy -c:s copy -metadata title='Tamil Fusion - t.me/TamilFusion1' -metadata:s:v title='Tamil Fusion - t.me/TamilFusion1' -metadata:s:a title='Tamil Fusion - t.me/TamilFusion1' -metadata:s:s title='Tamil Fusion - t.me/TamilFusion1' '{out_loc}' -y")
        if rcode != 0:
